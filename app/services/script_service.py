...
def chunk_turns_by_words(
    ...
    logger.info(f"[Chunking] Created {len(chunks)} chunks (first≈{first_words or words_per_chunk}, min_first={first_min_words}, rest≈{words_per_chunk}) from {len(turns)} turns")
    return chunks
...
"""
    ...
    try:
        ...
        chunks = chunk_turns_by_words(
            single_script["multiSpeakerMarkup"]["turns"], 
        ...
        return {
            "segments": [
                {"segmentId": idx + 1, "multiSpeakerMarkup": {"turns": seg}} for idx, seg in enumerate(chunks)
            ]
        }
