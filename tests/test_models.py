...
from app.models import PodcastRequest, ScriptChunkedResponse, ScriptSegment, TTSSegmentRequest, TTSSegmentResponse
...
class TestPodcastRequest:
    ...
    def test_valid_request(self):
        ...
        data = {"topic": "Test Topic", "minutes": 5}
...
class TestScriptSegment:
    ...
    def test_valid_segment(self):
        ...
        data = {
            "segmentId": 1,
            "multiSpeakerMarkup": {"turns": [{"text": "Hello", "speaker": "Jay"}, {"text": "Hi there", "speaker": "Nik"}]},
        }
    ...
    def test_camel_case_alias(self):
        ...
        data = {"segmentId": 2, "multiSpeakerMarkup": {"turns": [{"text": "Test", "speaker": "Jay"}]}}
        segment = ScriptSegment(**data)
    ...
    def test_missing_segment_id(self):
        ...
        data = {"multiSpeakerMarkup": {"turns": [{"text": "Hello", "speaker": "Jay"}]}}
        with pytest.raises(ValidationError):
            ScriptSegment(**data)
...
class TestScriptChunkedResponse:
    ...
    def test_valid_response(self):
        ...
        data = {
            "segments": [{"segmentId": 1, "multiSpeakerMarkup": {"turns": [{"text": "Hello", "speaker": "Jay"}]}}],
            "total_words": 100,
        }
...
class TestTTSSegmentRequest:
    ...
    def test_valid_request(self):
        ...
        data = {"segmentId": 1, "turns": [{"speaker": "Jay", "text": "Hello"}, {"speaker": "Nik", "text": "Hi there"}]}
    ...
    def test_camel_case_alias(self):
        """Test that camelCase field names work with alias"""
        data = {"segmentId": 2, "turns": [{"speaker": "Jay", "text": "Test"}]}
        request = TTSSegmentRequest(**data)
        assert request.segment_id == 2  # Should map from segmentId

    def test_missing_segment_id(self):
        """Test validation error when segment_id is missing"""
        data = {"turns": [{"speaker": "Jay", "text": "Test"}]}
        with pytest.raises(ValidationError):
    ...
    def test_invalid_turn_structure(self):
        ...
        data = {"segment_id": 1, "turns": [{"speaker": "Jay"}, {"text": "Hi there"}]}  # Missing text  # Missing speaker
        with pytest.raises(ValidationError):
            TTSSegmentRequest(**data)
...
class TestTTSSegmentResponse:
    ...
    def test_missing_base64(self):
        ...
        data = {"segment_id": 1, "mime": "audio/wav"}
        with pytest.raises(ValidationError):
            TTSSegmentResponse(**data)

    def test_empty_base64(self):
        """Test validation error when base64 is empty"""
        data = {"segment_id": 1, "mime": "audio/wav", "base64": ""}
        with pytest.raises(ValidationError):
            TTSSegmentResponse(**data)
