...
class TestAPIEndpoints:
    ...
    def test_generate_script_chunked_success(self, mock_generate, client):
            ...
            "segments": [
                {
                    "segmentId": 1,
                    "multiSpeakerMarkup": {
                        "turns": [{"text": "Hello", "speaker": "Jay"}, {"text": "Hi there", "speaker": "Nik"}]
                    },
                }
            ],
            "total_words": 10,
        ...
        response = client.post("/generate-script-chunked/", json={"topic": "Test Topic", "minutes": 5})
        assert response.status_code == 200
        data = response.json()
    ...
    def test_generate_script_chunked_invalid_request(self, client):
        ...
        response = client.post("/generate-script-chunked/", json={"topic": "", "minutes": 5})  # Invalid: empty topic
        assert response.status_code == 422

    @patch("app.main.generate_tts_segment")
    def test_tts_segment_success(self, mock_tts, client):
        """Test successful TTS generation"""
        mock_tts.return_value = {"segment_id": 1, "base64": "dGVzdA==", "mime": "audio/wav"}  # "test" in base64

        response = client.post(
            "/tts-segment/",
            json={"segmentId": 1, "turns": [{"text": "Hello", "speaker": "Jay"}, {"text": "Hi there", "speaker": "Nik"}]},
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_tts_segment_invalid_request(self, client):
        ...
        response = client.post("/tts-segment/", json={"segmentId": 1, "turns": []})  # Invalid: empty turns
        assert response.status_code == 422
    ...
    def test_tts_segment_retry_logic(self, mock_tts, client):
        ...
        mock_tts.side_effect = [Exception("First failure"), {"segment_id": 1, "base64": "dGVzdA==", "mime": "audio/wav"}]
